...
while currdist>Tolerance && tempcounter<=maxiter
    ...
    for z_c=1:N_z
        ...
        % ReturnMatrix_fine_raw(d,a',a) has size: [N_d,3+2*n2short,n_a]
        ReturnMatrix_fine_raw = CreateReturnFnMatrix_GI_lowmem(ReturnFn,d_gridvals,aprime_fine_small,a_gridvals,z_vals,ParamCell,n_z);
       
        % ReturnMatrix_fine(a',a) and dstar(a',a)
        [ReturnMatrix_fine,dstar_z] = max(ReturnMatrix_fine_raw,[],1);
        
        ReturnMatrix_fine = reshape(ReturnMatrix_fine,[n_aprime,n_a]);
        
        % EV_z_interp(a',a) has same size as aprime_fine_small
        ...
        [max_val,Policy_az]=max(entireRHS_fine,[],1); % (1,N_a)
        
        dstar(:,:,z_c)  = dstar_z;
        VKron(:,z_c)    = max_val;

        % Policy_a is currently index on fine grid, from 1 to 3+2*n2short
        Policy_a(:,z_c) = Policy_az;
...
elseif l_z==2
    Fmatrix=arrayfun(ReturnFn,d_gridvals,aprime_grid,a_grid, z_gridvals(1), z_gridvals(2), ParamCell{:});
elseif l_z==3
    Fmatrix=arrayfun(ReturnFn,d_gridvals,aprime_grid,a_grid, z_gridvals(1), z_gridvals(2), z_gridvals(3), ParamCell{:});
end
